...
• Imports train_gpu and launches training in a background daemon thread 3s after boot.
• Streams logs and refreshes reward plots every 5s via gr.Timer.
"""
...
def _status() -> str:
    ...
    if s == "running":
        ...
        return (f"Training — {n} completions / "
                f"{train_gpu.MAX_STEPS * train_gpu.NUM_GENERATIONS} total  |  "
                f"Rolling avg reward: {avg:.3f}  |  Level: {train_gpu.curriculum_level}")
    if s == "done":
...
def ui_tick(current_log: str):
    return (_collect_logs(current_log), _status(),
            _plot("reward_curve.png"), _plot("component_rewards.png"),
            _plot("localization_accuracy.png"), _plot("baseline_vs_trained.png"))
...
with gr.Blocks(
    ...
    status_box = gr.Textbox(value="Initializing — training starts in ~3 s...",
                            label="Training Status", interactive=False)

    with gr.Row():
        with gr.Column(scale=5):
            log_box = gr.Textbox(value="Waiting for training to start...",
                                 label="Training Logs", lines=30, max_lines=38,
    ...
    timer = gr.Timer(5.0)
    timer.tick(fn=ui_tick, inputs=[log_box],
