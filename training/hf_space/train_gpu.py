"""
CascadeDebug GRPO Training — HF Spaces L4 GPU
===============================================
All dependencies are pre-installed via requirements.txt (no runtime pip installs).
CUDA version mismatch is avoided by pinning torch/torchvision from the same index.
"""

import json, os, sys, csv, random, re, time, traceback
import queue as queue_module
from pathlib import Path

# ── Shared state (read by app.py) ─────────────────────────────────────────────
log_queue: queue_module.Queue = queue_module.Queue()
training_state: dict = {"status": "idle", "elapsed": 0.0}
reward_history: list = []
all_log_rows: list = []
PIPELINE_BANK: list = []
BANK_BY_LEVEL: dict = {}
...
def _resolve_grpo_precision(torch_module):
    """
    Unsloth 4-bit + GRPO: do not fp16-train or downcast LoRA — breaks matmul_lora (Half vs Float).
    Prefer bf16 when supported; else float32 + UNSLOTH_FORCE_FLOAT32 (Unsloth docs).
    """
...
def parse_action(text: str) -> dict:
    ...
    try:
        ...
        return {"fault_step_id": 1, "blame_role": "Researcher",
                "fix_content": "parse error", "action_type": "submit"}


# ── Reward functions ──────────────────────────────────────────────────────────
def reward_localization(predicted: int, true: int, level: int = 1) -> float:
    if predicted == true: return 1.0
    if level == 1 and abs(predicted - true) == 1: return 0.3
    return 0.0

def reward_blame(predicted: str, true: str) -> float:
    return 1.0 if predicted == true else 0.0

def reward_fix(fix_content, original_output, expected_keywords, corrupted_output) -> float:
    if not fix_content or not fix_content.strip(): return 0.0
    fl = fix_content.lower()
    kw_score = (sum(1 for kw in expected_keywords if kw.lower() in fl) / len(expected_keywords)
    ...
    if original_output:
        ...
        length_score = 1.0 if 0.3 <= ratio <= 3.0 else max(0.0, min(ratio/0.3, 1.0-(ratio-3.0)/7.0))
        orig_words = set(original_output.lower().split())
        fix_words = set(fl.split())
        sim_score = len(orig_words & fix_words) / max(len(orig_words), 1)
    else:
        length_score, sim_score = 0.5, 0.0
    return min(max(0.50*kw_score + 0.20*nc_score + 0.15*length_score + 0.15*sim_score, 0.0), 1.0)

def reward_precision(turn=1, gatekeeper_accepted=False) -> float:
    if gatekeeper_accepted: return 1.0 if turn == 1 else 0.6
    return 0.2

def compute_episode_reward(action, episode, level=1) -> dict:
    ...
    r3 = reward_fix(action["fix_content"], episode.get("original_output", ""),
                     episode.get("expected_fix_keywords", []), episode.get("corrupted_output", ""))
...
def cascadedebug_reward(completions, **kwargs):
    ...
    for i, completion in enumerate(completions):
        ...
        all_log_rows.append({
            "step": len(reward_history), "level": curriculum_level,
            "r_loc": result["localization"], "r_blame": result["blame"],
            "r_fix": result["fix"], "r_prec": result["precision"], "r_total": total,
            "predicted_step": action["fault_step_id"], "true_step": ep["injected_step"],
            "predicted_role": action["blame_role"], "true_role": ep["injected_role"],
        })
    ...
    if (len(reward_history) >= PLOT_EVERY * NUM_GENERATIONS and
        ...
        try: _save_interim_plot()
        except Exception as e: log(f"⚠️ Interim plot error: {e}")

    return rewards


# ── Plot helpers ──────────────────────────────────────────────────────────────
def _save_interim_plot():
    import numpy as np
    import matplotlib; matplotlib.use("Agg")
    import matplotlib.pyplot as plt
    if len(reward_history) < 5: return
    ...
    if len(reward_history) > 10:
        ...
        ax.plot(range(w-1, len(reward_history)), rolling, color="#ef4444", linewidth=2, label=f"Rolling avg (w={w})")
    avg = sum(reward_history[-ROLLING_WINDOW:]) / min(len(reward_history), ROLLING_WINDOW)
    ax.set_xlabel("Training Completion"); ax.set_ylabel("Total Reward")
    ax.set_title(f"CascadeDebug GRPO | Completions: {len(reward_history)} | Avg: {avg:.3f} | Level: {curriculum_level}",
                 fontsize=12, fontweight="bold")
    ax.legend(); ax.grid(True, alpha=0.3); ax.set_ylim(0, 1)
    plt.tight_layout(); plt.savefig(RESULTS_DIR / "reward_curve.png", dpi=110); plt.close(fig)
...
def generate_final_plots():
    ...
    if reward_history:
        ...
        if len(reward_history) > 20:
            ...
            rolling = np.convolve(reward_history, np.ones(w)/w, mode="valid")
            ax.plot(range(w-1, len(reward_history)), rolling, color="#ef4444", linewidth=2.5, label=f"Rolling avg (w={w})")
        ax.set_xlabel("Training Completion", fontsize=13); ax.set_ylabel("Total Reward", fontsize=13)
        ax.set_title("CascadeDebug — GRPO Training Reward Curve", fontsize=15, fontweight="bold")
        ax.legend(fontsize=11); ax.grid(True, alpha=0.3); ax.set_ylim(0, 1)
        plt.tight_layout(); plt.savefig(RESULTS_DIR / "reward_curve.png", dpi=150); plt.close(fig)
        log("   ✅ reward_curve.png")

    if all_log_rows:
        fig, axes = plt.subplots(2, 2, figsize=(14, 9))
        components = [("r_loc","Fault Localization (r1, w=0.35)","#3b82f6"),
        ...
        for ax, (key, title, color) in zip(axes.flat, components):
            ...
            ax.set_xlabel("Step"); ax.set_ylabel("Reward"); ax.set_ylim(-0.05, 1.05); ax.grid(True, alpha=0.3)
        plt.suptitle("CascadeDebug — Component Reward Signals", fontsize=14, fontweight="bold")
        plt.tight_layout(); plt.savefig(RESULTS_DIR / "component_rewards.png", dpi=150); plt.close(fig)
    ...
    if all_log_rows:
        ...
        if len(correct) > 20:
            ...
            ax.plot(range(w-1, len(correct)), rolling, color="#3b82f6", linewidth=2, label="Rolling accuracy")
        ax.axhline(y=1/3, color="#ef4444", linestyle="--", linewidth=1.5, label="Random baseline (1/3)")
        ax.set_xlabel("Step", fontsize=13); ax.set_ylabel("Localization Accuracy", fontsize=13)
        ax.set_title("CascadeDebug — Fault Localization Accuracy", fontsize=15, fontweight="bold")
        ax.set_ylim(0, 1); ax.legend(fontsize=11); ax.grid(True, alpha=0.3)
        plt.tight_layout(); plt.savefig(RESULTS_DIR / "localization_accuracy.png", dpi=150); plt.close(fig)
        log("   ✅ localization_accuracy.png")

    if all_log_rows and len(all_log_rows) > 20:
        import numpy as np
        fig, ax = plt.subplots(figsize=(10, 6))
        n = len(all_log_rows); n5 = max(n // 5, 10)
        for label, rows, offset, color in [
            ("Untrained (first 20%)", all_log_rows[:n5], -0.15, "#94a3b8"),
            ("Trained (last 20%)",    all_log_rows[-n5:], 0.15, "#3b82f6")]:
            by_level = {}
            for r in rows: by_level.setdefault(r["level"], []).append(r["r_total"])
        ...
        plt.tight_layout(); plt.savefig(RESULTS_DIR / "baseline_vs_trained.png", dpi=150); plt.close(fig)
...
def push_to_hub(model, tokenizer):
    ...
    try:
        ...
        for fname in ["reward_curve.png", "component_rewards.png",
                      "localization_accuracy.png", "baseline_vs_trained.png", "training_log.csv"]:
            fpath = RESULTS_DIR / fname
            if fpath.exists():
                try:
                    api.upload_file(path_or_fileobj=str(fpath), path_in_repo=f"results/{fname}",
                                    repo_id=HF_RESULTS_REPO, repo_type="space",
        ...
        try:
            ...
            try:
                api.create_repo(repo_id="Dikshita2026/cascadedebug-model", repo_type="model", exist_ok=True)
                api.upload_folder(folder_path=merged_dir, repo_id="Dikshita2026/cascadedebug-model",
                                  repo_type="model", commit_message="CascadeDebug: merged GRPO-trained model")
...
def run() -> None:
    ...
    try:
        ...
        if not torch.cuda.is_available():
            raise RuntimeError("No CUDA GPU detected! Set Space hardware to L4.")
        ...
        if os.environ.get("UNSLOTH_FORCE_FLOAT32") == "1":
            log("   Precision: float32 + UNSLOTH_FORCE_FLOAT32 (bf16 not available)")
        else:
        ...
        model, tokenizer = FastLanguageModel.from_pretrained(
            ...
            load_in_4bit=True, dtype=compute_dtype, fast_inference=False,
        )
        ...
        training_args = GRPOConfig(
            ...
            learning_rate=LEARNING_RATE, per_device_train_batch_size=1,
            ...
            max_steps=MAX_STEPS, logging_steps=LOG_EVERY, save_steps=SAVE_EVERY,
            fp16=use_fp16, bf16=use_bf16,
            gradient_accumulation_steps=GRADIENT_ACCUM, warmup_ratio=0.1,
            optim="adamw_8bit", seed=42, report_to="none",
            remove_unused_columns=False, log_level="warning",
        ...
        log(f"\n✅ Training done in {elapsed/60:.1f} min ({len(reward_history)} reward samples)")
        ...
        if reward_history:
            n = len(reward_history); n5 = max(n // 5, 1)
            ...
            if all_log_rows:
                ...
                loc_acc = sum(1 for r in late_rows if r["predicted_step"]==r["true_step"]) / max(len(late_rows),1)
